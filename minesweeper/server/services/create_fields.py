...
def create_field_mines(game: Game, validated_data: dict) -> None:
    coordinates = _get_coordinates(game)
    field_mines = create_field(validated_data, 0)
...
def _add_around_cnt(
    ...
    y_max = min(coord.y_height + 2, len(field[0]))
...
def _get_coordinates(game: Game) -> Set[Coord]:
    return {Coord(coord.x_width, coord.y_height) for coord in game.mines.all()}
